...
a. Fixed a major bug in :py:func:`pingouin.pairwise_ttests` when using mixed design (= specifiying both a within-subject and a between-subject factor). Specifically, the between-subject T-tests were perfomed without averaging over the repeated measurements. This led to higher degrees of freedom (because they were conflated with the number of repeated measurements) and ultimately incorrect p-values. Pingouin now averages over repeated measurements, which is the same behavior as JASP or JAMOVI. As a consequence, and when the data has only two groups, the between-subject p-value of the pairwise T-test should be (almost) equal to the p-value of the same factor in the :py:func:`pingouin.mixed_anova` function. **We strongly recommend that you update to the newest version of Pingouin and double-check your previous code if you've ever worked with mixed-design pairwise T-tests.**
b. Added a check in :py:func:`pingouin.mixed_anova` to ensure that the ``subject`` variable has a unique set of values for each between-subject group defined in the ``between`` variable. For instance,  the subject IDs for group1 are [1, 2, 3, 4, 5] and for group2 [6, 7, 8, 9, 10]. The function will throw an error if there are one or more overlapping subject IDs between groups (e.g. the subject IDs for group1 AND group2 are both [1, 2, 3, 4, 5]).
c. Fixed a minor bug which caused the :py:func:`pingouin.plot_rm_corr` to throw an error if any of the input variables started with a number (because of Patsy formula formatting).
