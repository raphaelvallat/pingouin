...
def plot_paired(data=None, dv=None, within=None, subject=None, order=None,
    ...
    _x = within if orient == 'v' else dv
    _y = dv if orient == 'v' else within

    for cat in range(len(x_cat) - 1):
        _order = (order[cat], order[cat + 1])
        # Extract data of the current subject-combination
        data_now = data.loc[data[within].isin(_order), [dv, within, subject]]
        # Select colors for all lines between the current subjects
        y1 = data_now.loc[data_now[within] == _order[0], dv].to_numpy()
        y2 = data_now.loc[data_now[within] == _order[1], dv].to_numpy()
        _colors = np.where(
            y1 < y2, colors[0], np.where(
                y1 > y2, colors[2], colors[1]))

        # Plot individual lines using Seaborn
        sns.pointplot(data=data_now, x=_x, y=_y, hue=subject, order=order,
                      palette=_colors, ax=ax, orient=orient,
                      **_pointplot_kwargs)

    # Remove legend entries and legend generated by pointplot
    _ = [_cl.set_label(s=None) for _cl in ax.collections]
    ...
    if boxplot:
        sns.boxplot(data=data, x=_x, y=_y, order=order, ax=ax,
