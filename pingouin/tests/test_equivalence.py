...
class TestEquivalence(TestCase):
    ...
    def test_tost(self):
        ...
        assert np.greater(tost(a, list(np.asarray(a) + 25),
                               bound=1).loc['TOST', 'p-val'], 0.5)
        a = [4, 7, 8, 6, 3, 2]
        b = [6, 8, 7, 10, 11, 9]
        assert round(tost(a, b, bound=10).loc['TOST', 'p-val'], 3) == 0.000
        assert round(tost(a, b, bound=10,
                          parametric=False).loc['TOST', 'p-val'], 3) == 0.003
        assert round(tost(a, b, bound=10, parametric=False,
                          paired=True).loc['TOST', 'p-val'], 3) == 0.018
    ...
    def test_againstRtost(self):
        ...
        assert round(tost(a, b, bound=1,
                          correction=True).loc['TOST', 'p-val'], 3) == 0.964
        assert round(tost(a, b, bound=1, correction=True,
                          paired=True).loc['TOST', 'p-val'], 3) == 0.929
        assert round(tost(a, b, bound=2, correction=False,
                          paired=True).loc['TOST', 'p-val'], 3) == 0.829
