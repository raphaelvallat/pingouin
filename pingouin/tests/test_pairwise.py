...
class TestPairwise(TestCase):
    ...
    def test_pairwise_tests(self):
        ...
        assert np.array_equal(pt1.loc[:5, 'p-corr'].round(3), [1., 1., 0.313, 1., 0.889, 1.])
        ...
        assert pt.equals(df_aov2.pairwise_tests(
            dv='Yield', between=['Blend', 'Crop'], padjust='holm', marginal=True).round(3))
        # -------------------------------------------------------------------
        ...
        pairwise_tests(
            dv='DesireToKill', within=['Disgustingness', 'Frighteningness'],
    ...
    def test_pairwise_tukey(self):
        ...
        assert np.allclose([0.074, 0.435, 0.415, 0.004, 0.789, 0.037],
                           stats.loc[:, 'p-tukey'].to_numpy().round(3), atol=0.05)
    ...
    def test_pairwise_gameshowell(self):
        ...
        assert np.allclose([0.1401, 0.5228, 0.3715, 0.0148, 0.6980, 0.0378],
                           stats.loc[:, 'pval'].to_numpy().round(3), atol=0.05)
        ...
        stats = pairwise_gameshowell(
            data=df_balanced, dv="body_mass_g", between="species").round(4)
        assert np.array_equal(stats['A'], ["Adelie", "Adelie", "Chinstrap"])
        assert np.array_equal(stats['B'], ["Chinstrap", "Gentoo", "Gentoo"])
        assert np.array_equal(stats['diff'], [-142.5, -1457.5, -1315.])
    ...
    def test_pairwise_corr(self):
        ...
        jasp_rval = [-0.350, -0.01, -.134, -.368, .267, .055, .065, .159, -.013, .159]
