...
class TestPairwise(TestCase):
    ...
    def test_pairwise_gameshowell(self):
        ...
        assert "pval" in stats.columns, "The 'pval' column is missing from the results"
    ...
    def test_pairwise_tests(self):
        ...
        # Ensure 'p_unc' and 'p_corr' columns exist
        assert "p_unc" in pt.columns, "The 'p_unc' column is missing from the results"
        assert "p_corr" in pt.columns, "The 'p_corr' column is missing from the results"
        ...
        # Ensure 'p_unc', 'p_corr' and other necessary columns exist
        assert "p_unc" in pt.columns, "The 'p_unc' column is missing from the results"
        assert "p_corr" in pt.columns, "The 'p_corr' column is missing from the results"
        assert "BF10" in pt.columns, "The 'BF10' column is missing from the results"
